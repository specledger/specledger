...
- [ ] Run bd ready --limit 5
- [ ] Report: "X items ready to work on: [summary]"
- [ ] If using global ~/.beads, note this in report
- [ ] If none ready, check bd blocked
- [ ] Suggest next action based on findings
```

**Pattern**: Always run `bd ready --limit 5` when starting work where bd is available. Report status immediately to establish shared context.
...
- [ ] Run bd show <issue-id> && bd comments <issue-id> for each in_progress issue
...
- [ ] Check what this issue blocks: bd dep tree <issue-id> --direction up
...
**Writing comments for compaction survival:**

**Good comment (enables recovery):**
```
bd comments add bd-123 "COMPLETED: User authentication - added JWT token
...
**Bad comment (insufficient for recovery):**
```
bd comments add bd-123 "Working on auth feature. Made some progress.
More to do later."
```

The good comment contains:
...
2. Create child tasks - use design and acceptance flags:
   bd create "Set up OAuth client credentials" -t task --parent <epic-id> --design "HOW To build, can change" --acceptance "WHAT success looks like"
   bd create "Implement authorization code flow" -t task --parent <epic-id> --design ... --acceptance ...
   bd create "Add token storage and refresh" -t task --parent <epic-id> --design ... --acceptance ...
   bd create "Create login/logout endpoints" -t task --parent <epic-id> --design ... --acceptance ...
...
```
Unblocking Workflow:
- [ ] Run bd blocked to see what's stuck
