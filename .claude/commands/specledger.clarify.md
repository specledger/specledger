...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `/specledger.plan`. If the user explicitly states they are skipping clarification (e.g., exploratory spike), you may proceed, but must warn that downstream rework risk increases.
...
1. Run `.specledger/scripts/bash/check-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
 ...
 2. **Fetch unresolved reviewer comments** (if available):
    ...
    - If exit code is 0 and output is non-empty JSON array, parse the comments.
...
4. Ask the user question with the ask user question tool for their preferences with details such as:
    - For multiple‑choice questions:
       ...
       - Include your **recommended option** together with the question with clear reasoning (1-2 sentences explaining why this is the best choice).
       - Then ask user which option they prefer.
    - For short‑answer style (no meaningful discrete options):
       - Include your **suggested answer** based on best practices and context together with the question
    - After the user answers:
       - If ambiguous, ask for a quick disambiguation follow up question.
...
8. Report completion (after questioning loop ends or early termination):
   ...
   - If any Outstanding or Deferred remain, recommend whether to proceed to `/specledger.plan` or run `/specledger.clarify` again later post-plan.
