...
**Created**: 2026-03-04
**Status**: Draft
**Input**: User description: "Create /specledger.commit slash command for auth-aware commit workflow. When user asks agent to commit (e.g. 'Commit and push to github'), agent runs this command. No auth → still push, skip capture silently. No project ID → still push, skip capture. Session upload fails → queue for sl session sync retry + send error to Sentry for troubleshooting."

## Problem Statement (V2)
Currently the AI agent (Claude Code) auto-commits and pushes code directly when the user asks "Commit and push to github". This causes three problems:
...
2. The error is logged locally (to `~/.specledger/capture-errors.log`) so the user can review it
3. The error is sent to Sentry so the team can troubleshoot remotely (aggregated, searchable, alertable)

Local log is always written first (guaranteed). Sentry is best-effort — if Sentry fails, the local log still has the full record.
...
**Independent Test**: Can be tested by simulating upload failure with valid credentials and project ID, then checking the local log file and Sentry dashboard.

**Acceptance Scenarios**:

1. **Given** a user is authenticated with a valid project ID, **When** session capture upload fails, **Then** the session is queued locally, the error is written to a local log file, AND the error is sent to Sentry (with user ID, project ID, error message, timestamp, session ID).
2. **Given** a queued session exists, **When** the user runs `sl session sync`, **Then** the system retries uploading queued sessions. If retry fails, the error is appended to local log and sent to Sentry.
3. **Given** a user is authenticated with a valid project ID, **When** token refresh fails during capture, **Then** the session is queued, error logged locally and sent to Sentry.
4. **Given** a team admin wants to troubleshoot session capture failures, **When** they check Sentry, **Then** they can see errors filtered by user/project with aggregation, trends, and alerting.
...
3. **Given** a user has credentials and project ID, **When** a git commit triggers the hook and capture fails, **Then** the session is queued and error is sent to Sentry.
...
- What happens when Sentry send itself fails? Local log file already has the error (written first). Do not block the workflow.
...
- **FR-006**: When session capture fails for an authenticated user with a valid project ID, the system MUST: (a) display the error to the user, (b) write the error to a local log file, AND (c) send the error to Sentry. All three happen on every failure.
- **FR-007**: Failed session captures MUST be queued locally for retry via `sl session sync`. When `sl session sync` retries and fails again, the retry error MUST also be logged locally and sent to Sentry.
...
- **FR-012**: Error logs MUST include: user ID, project ID, error message, timestamp, and session ID for correlation. This applies to BOTH local log file and Sentry.
- **FR-013**: Error logging MUST always write to local log file first (guaranteed), then attempt Sentry. If Sentry send fails, the local log still has the full record. Logging failures MUST never block the commit/push workflow.
...
- **SC-005**: All capture errors for authenticated users are persisted to BOTH local log file AND Sentry (searchable, aggregatable).
