...
1. **Given** a user runs `sl init` in a new project directory, **When** the command completes, **Then** a .claudeignore file is created with sensible defaults for common build artifacts, dependencies, and temporary files.

2. **Given** a user runs `sl new <project-name>`, **When** the project is scaffolded, **Then** the generated project includes a pre-configured .claudeignore file.

3. **Given** a .claudeignore file already exists, **When** running `sl init`, **Then** the existing file is preserved and not overwritten.
...
2. **Given** a project with multiple agent types (Claude, other AI tools), **When** I configure ignore patterns, **Then** the patterns apply appropriately to each agent.
...
- **FR-006**: System MUST document the .claudeignore format and available patterns
...
2. If not, create with default patterns
3. If yes, skip generation (preserve user customizations)
4. Log action to user
